    ...
    <el-form style="margin-left: -40px" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm">
      <el-form-item label="字段：" prop="key">
        <el-select v-model="ruleForm.key" style="width: 160px;float: left" placeholder="请选择字段">
          ...
          <el-option label="考查月份" value="month"></el-option>
        </el-select>
      </el-form-item>
...
export default {
  ...
  methods:{
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:8181/performance/search',{params:_this.ruleForm}).then(function (resp) {
    ...
    page(currentPage){
      ...
      if(_this.ruleForm.value == ''){
        axios.get('http://localhost:8181/performance/list/'+currentPage+'/'+_this.pageSize+'/'+this.admin.companyID).then(function (resp) {
          _this.tableData = resp.data.data.data
          _this.total = resp.data.data.total
        })
      } else {
  ...
  created() {
    ...
    this.admin = admin
    axios.get('http://localhost:8181/performance/list/1/'+_this.pageSize+'/'+this.admin.companyID).then(function (resp) {
      _this.tableData = resp.data.data.data
      _this.total = resp.data.data.total
    })
  }
