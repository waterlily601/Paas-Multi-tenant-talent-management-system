    ...
    <el-form style="margin-left: -40px" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      <el-form-item label="字段：" prop="key">
        <el-select v-model="ruleForm.key" style="width: 160px;float: left" placeholder="请选择字段">
          <el-option label="HRID" value="hrID"></el-option>
          <el-option label="HR账号" value="username"></el-option>
          <el-option label="HR姓名" value="hrname"></el-option>
        </el-select>
      </el-form-item>
  ...
  methods:{
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:8181/hradmin/search',{params:_this.ruleForm}).then(function (resp) {
    ...
    page(currentPage){
      ...
      if(_this.ruleForm.value == ''){
        axios.get('http://localhost:8181/hradmin/list/'+currentPage+'/'+_this.pageSize+'/'+this.admin.companyID).then(function (resp) {
          _this.tableData = resp.data.data.data
          _this.total = resp.data.data.total
        })
      } else {
    ...
    edit(row){
      this.$router.push('/hrUpdate?id='+row.hrID)
    },
    del(row) {
      const _this = this
      this.$confirm('确认删除【'+row.username+'】？', '提示', {
        ...
        axios.delete('http://localhost:8181/hradmin/deleteById/'+row.hrID).then(function (resp) {
          if(resp.data.code==0){
